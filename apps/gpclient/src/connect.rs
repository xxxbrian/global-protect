...
pub(crate) struct ConnectArgs {
    ...
    #[arg(
        short = 'k',
        long,
        help = "Use SSL private key file in pkcs#8 (.pem) format"
    )]
    ...
    #[arg(
        long,
        help = "Same as the '--csd-user' option in the openconnect command"
    )]
    csd_user: Option<String>,

    #[arg(
        long,
        help = "Same as the '--csd-wrapper' option in the openconnect command"
    )]
    ...
    #[arg(
        long,
        default_value = "300",
        help = "Reconnection retry timeout in seconds"
    )]
    ...
    #[arg(
        long,
        help = "If not specified, it will be computed based on the --os option"
    )]
...
impl<'a> ConnectHandler<'a> {
    ...
    pub(crate) async fn handle(&self) -> anyhow::Result<()> {
        ...
        self.latest_key_password
            .replace(self.args.key_password.clone());
        ...
        loop {
            ...
            match root_cause {
                ...
                RequestIdentityError::NoPassphrase(cert_type)
                | RequestIdentityError::DecryptError(cert_type) => {
    ...
    async fn connect_gateway(&self, gateway: &str, cookie: &str) -> anyhow::Result<()> {
        ...
        let os = ClientOs::from(&self.args.os)
            .to_openconnect_os()
            .to_string();
    ...
    async fn obtain_credential(
        ...
        match prelogin {
            Prelogin::Saml(prelogin) => {
                ...
                #[cfg(feature = "webview-auth")]
                let use_default_browser =
                    prelogin.support_default_browser() && self.args.default_browser;
                #[cfg(feature = "webview-auth")]
