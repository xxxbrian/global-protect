...
impl AuthMessenger {
    ...
    pub fn read_from_html(&self, html: &str) {
        if html.contains("Temporarily Unavailable") {
            return self.send_auth_error(AuthError::invalid_from_body(anyhow::anyhow!(
        ...
        let auth_result = SamlAuthData::from_html(html).or_else(|err| {
            info!(
                "Read auth data from html failed: {}, extracting gpcallback...",
                err
            );
        ...
        match auth_result {
            ...
            Err(AuthDataParseError::Invalid(err)) => {
                self.send_auth_error(AuthError::invalid_from_body(err))
            }
            Err(AuthDataParseError::NotFound) => {
                self.send_auth_error(AuthError::not_found_in_body())
            }
        }
    }
