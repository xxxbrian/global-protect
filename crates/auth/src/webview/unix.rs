...
use webkit2gtk::{
    gio::Cancellable, glib::GString, LoadEvent, TLSErrorsPolicy, URIResponseExt, WebResource,
    WebResourceExt, WebViewExt, WebsiteDataManagerExt,
};
...
impl PlatformWebviewOnResponse for PlatformWebview {
    fn on_response(&self, callback: Box<dyn Fn(anyhow::Result<WebResource, AuthError>) + 'static>) {
        ...
        wv.connect_load_changed(move |wv, event| {
            ...
            if uri.is_empty() {
                callback_clone(Err(AuthError::invalid_from_body(anyhow::anyhow!(
                    "Empty URI"
                ))));
            } else {
