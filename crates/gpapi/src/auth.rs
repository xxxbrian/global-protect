...
impl SamlAuthData {
    ...
    pub fn from_html(html: &str) -> AuthDataParseResult {
        match parse_xml_tag(html, "saml-auth-status") {
            Some(status) if status == "1" => {
                ...
                SamlAuthData::new(username, prelogin_cookie, portal_userauthcookie)
                    .map_err(AuthDataParseError::Invalid)
            }
    ...
    pub fn from_gpcallback(data: &str) -> anyhow::Result<SamlAuthData, AuthDataParseError> {
        ...
        if auth_data.starts_with("cas-as") {
            ...
            let auth_data: SamlAuthData =
                serde_urlencoded::from_str(auth_data.borrow()).map_err(|e| {
...
mod tests {
    ...
    fn auth_data_from_gpcallback_cas_urlencoded() {
        let auth_data =
            "globalprotectcallback:cas-as%3D1%26un%3Dxyz%40email.com%26token%3Dvery_long_string";
