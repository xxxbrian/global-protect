...
impl Credential {
    ...
    pub fn to_params(&self) -> HashMap<&str, &str> {
        ...
        let (passwd, prelogin_cookie, portal_userauthcookie, portal_prelogonuserauthcookie, token) =
            match self {
                Credential::Password(cred) => (Some(cred.password()), None, None, None, None),
                Credential::Prelogin(cred) => {
                    (None, cred.prelogin_cookie(), None, None, cred.token())
                }
                ...
                Credential::Cached(cred) => (
                    ...
                    cred.auth_cookie
                        .as_ref()
                        .map(|c| c.prelogon_user_auth_cookie()),
                    None,
                ),
            };
