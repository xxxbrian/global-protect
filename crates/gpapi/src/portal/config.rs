    ...
    pub fn find_preferred_gateway(&self, region: &str) -> &Gateway {
        ...
        for gateway in &self.gateways {
            for rule in &gateway.priority_rules {
                if (rule.name == region || rule.name == "Any")
                    && rule.priority < lowest_region_priority
                {
...
pub async fn retrieve_config(
    ...
    info!(
        "Retrieve the portal config, user_agent: {}",
        gp_params.user_agent()
    );
    ...
    let user_auth_cookie = root
        .descendant_text("portal-userauthcookie")
        .unwrap_or_default();
    ...
    Ok(PortalConfig {
        ...
        internal_host_detection: if ihd_enabled {
            Some(prefer_internal)
        } else {
            None
        },
    })
...
fn internal_host_detect(node: &Node) -> bool {
    let ip_info = [
        ...
        (
            node.child_text("ipv6-address"),
            node.child_text("ipv6-host"),
        ),
    ];
