...
fn create_identity_from_pem(
    cert: &str,
    key: Option<&str>,
    passphrase: Option<&str>,
) -> anyhow::Result<Identity> {
    let cert_pem = fs::read(cert)
        .map_err(|err| anyhow::anyhow!("Failed to read certificate file: {}", err))?;

    // Use the certificate as the key if no key is provided
    let key_pem_file = match key {
        Some(key) => Cow::Owned(
            fs::read(key).map_err(|err| anyhow::anyhow!("Failed to read key file: {}", err))?,
        ),
    ...
    let decrypted_key_pem = if key_pem.tag().ends_with("ENCRYPTED PRIVATE KEY") {
        ...
        let key =
            PKey::private_key_from_pem_passphrase(pem_content.as_bytes(), passphrase.as_bytes())
                .map_err(|err| {
